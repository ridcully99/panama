 ...
 */
public class MyBot15_sections extends Bot {
    ...
    public static void main(String[] args) throws IOException {
    	MyBot15_sections myBot = new MyBot15_sections();
	 ...
	 */
	private int wouldSurviveFight(Tile myAnt) {
		Set<Tile> enemies = getEnemiesInRange(myAnt, 0, extendedAttackRadius2);
		if (enemies.size() == 0) {
		...
		for (Tile enemy : enemies) {
			...
			// TODO optimize!
			List<Aim> directions = ants.getDirections(enemy, myAnt);
			Set<Tile> enemiesEnemiesInSection = new HashSet<Tile>();
			for (Tile ee : enemiesEnemies) {
				List<Aim> eeDirections = ants.getDirections(enemy, ee);
				eeDirections.removeAll(directions);
				if (!eeDirections.isEmpty()) {
					enemiesEnemiesInSection.add(ee);
				}
			}
			log("enemiesEnemiesInSection.size "+enemiesEnemiesInSection.size());
			if (enemiesEnemiesInSection.size() < enemies.size()) {
				result = I_WOULD_DIE;
				break;
			}
	...
	private Aim calcFight(Tile myAnt) {
		Set<Aim> goodAims = new HashSet<Aim>();

		Set<Tile> enemies = getEnemiesInRange(myAnt, 0, extendedAttackRadius2);
		float enemiesRate = rate(myAnt, enemies);
		for (Tile enemy : enemies) {
			timeoutCheck();
			Set<Tile> enemiesEnemies = getEnemiesInRange(enemy, antOwners.get(enemy), extendedAttackRadius2);
			// TODO optimize!
			...
			for (Tile ee : enemiesEnemies) {
				List<Aim> eeDirections = ants.getDirections(enemy, ee);
				// wenn Durchschnitt von eeDirections und directions nicht empty, dann zu enemies in Section hinzufuegen.
				// FALSCH!--> eeDirections.removeAll(directions);
				if (!Collections.disjoint(directions, eeDirections)) {
					enemiesEnemiesInSection.add(ee);
				}
			}
			log("enemiesEnemiesInSection.size "+enemiesEnemiesInSection.size());
			if (rate(enemy, enemiesEnemiesInSection) <= enemiesRate) {
				goodAims.removeAll(ants.getDirections(myAnt, enemy));
			} else {
				goodAims.addAll(ants.getDirections(myAnt, enemy));
			}
		}
		
 		for (Aim direction : goodAims) {
 			Tile dest = ants.getTile(myAnt, direction);
 			if (ants.getIlk(dest).isPassable() && 
 				(ants.getIlk(dest).isUnoccupied() || ants.getMyAnts().contains(dest)) &&
 				!blocked.contains(dest) &&
	...
	private float rate(Tile location, Set<Tile> others) {
		float rate = 0;
		for (Tile other : others) {
			int d = ants.getDistance(location, other);
			if (d <= 10) {
				rate++;
			} else if (d <= 17) {
				rate += 0.25;
